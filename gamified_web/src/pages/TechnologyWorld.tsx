          ...
          {viewMode === 'domain-map' && (
            <motion.div key="domain-map" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }}
              ...
              <div className="w-full lg:w-5/12 flex flex-col justify-between p-6 rounded-2xl border border-purple-500/10 bg-[#0c0517]/80 backdrop-blur-md shadow-2xl relative overflow-hidden">
                ...
                <div>
                  <span className="font-display text-xs tracking-widest text-[#7b2fff] uppercase block mb-1">{t('stageMap.centralRelic', 'Central Relic')}</span>
                  <h2 className="font-display text-xl tracking-wider text-slate-100 uppercase">{t('stones.technologyStone', 'The Circuit Stone')}</h2>
                  <p className="text-sm text-slate-400 mt-2 leading-relaxed">{t('stageMap.techDesc', 'Master binary logic, programming, algorithms, web & cybersecurity. Complete all 4 Stage Bosses to unlock the Technology Stone!')}</p>
                </div>
                ...
                <div className="border-t border-slate-800/40 pt-4 flex justify-between items-center text-sm text-slate-500">
                  ...
                  <span className="font-display text-[#7b2fff] font-bold">{[1,2,3,4].filter(n => isStageCompleted(n)).length} / 4 {t('stageMap.secured', 'SECURED')}</span>
                </div>
              </div>
              ...
              <div className="w-full lg:w-7/12 flex flex-col gap-4">
                {[
                  { num: 1, title: `${t('stageMap.stage', 'STAGE')} 1 — ${t('stageMap.discover', 'DISCOVER')}`, desc: t('stageMap.techDesc1', 'Learn computer basics, hardware vs software, binary counting, input/output devices, and file management.'), locked: false },
                  { num: 2, title: `${t('stageMap.stage', 'STAGE')} 2 — ${t('stageMap.understand', 'UNDERSTAND')}`, desc: t('stageMap.techDesc2', 'Master algorithms, flowcharts, pseudocode, block programming, variables, conditionals, and loops.'), locked: !isStageUnlocked(2) },
                  { num: 3, title: `${t('stageMap.stage', 'STAGE')} 3 — ${t('stageMap.lifeEarth', 'ADVANCED')}`, desc: t('stageMap.techDesc3', 'Explore the internet, web pages, HTML/CSS basics, digital safety, cyber threats, and online privacy.'), locked: !isStageUnlocked(3) },
                  { num: 4, title: `${t('stageMap.stage', 'STAGE')} 4 — ${t('stageMap.mastery', 'MASTERY')}`, desc: t('stageMap.techDesc4', 'Master software engineering, AI concepts, data structures, cloud computing, and real-world tech systems.'), locked: !isStageUnlocked(4) }
                ].map(st => {
                    ...
                    <div key={st.num} className={`p-5 rounded-2xl border transition-all duration-300 relative overflow-hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${completed ? 'border-purple-500/20 bg-purple-500/5' : st.locked ? 'border-slate-800 bg-[#05020c]/40 opacity-55' : 'border-purple-500/20 bg-[#0c0517]/80 hover:border-purple-500/40 hover:bg-[#120824]/80'}`}>
                      <div className="max-w-[420px]">
                        <div className="flex items-center gap-2">
                          <span className="font-display text-xs tracking-wider font-semibold text-[#7b2fff] uppercase">{t('stageMap.stage', 'STAGE')} {st.num}</span>
                          {completed && <span className="px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 font-display text-xs font-bold tracking-widest uppercase">✓ {t('stageMap.completed', 'Completed')}</span>}
                        </div>
          ...
          {viewMode === 'stage-levels' && (
            <motion.div key="stage-levels" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="text-center mb-8 max-w-xl">
                <span className="font-display text-xs tracking-widest text-[#7b2fff] uppercase block mb-1">{t('stageMap.stage', 'STAGE')} {selectedStageNum}</span>
