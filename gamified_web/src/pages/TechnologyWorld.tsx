...
import { TECHNOLOGY_LEVELS, getLevelsByStage } from '../data/technologyLevels';
import type { TechLevel } from '../data/technologyLevels';
import { audioSynth } from '../utils/audio';

// ── ENVIRONMENT PER STAGE ─────────────────────────────────
const getMissionEnvironment = (level: TechLevel) => {
  if (level.stage === 1) {
    return {
      name: "Digital Control Bay",
      themeColor: "#7b2fff",
      icon: "💻",
      stageTag: "STAGE 1 — FOUNDATIONS",
      ambientSpots: (
        <>
          <div className="absolute top-10 left-10 w-96 h-96 rounded-full bg-[#7b2fff]/5 blur-[120px] pointer-events-none" />
      ...
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-20 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'linear-gradient(rgba(123,47,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(123,47,255,0.04) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
          <div className="absolute top-1/4 left-1/3 w-2 h-2 rounded-full bg-[#7b2fff] animate-ping" />
          <div className="absolute top-2/3 right-1/4 w-2.5 h-2.5 rounded-full bg-purple-400 animate-ping" style={{ animationDelay: '1.5s' }} />
        </div>
      ),
      missionSuccessText: "💻 CODE EXECUTED!",
      missionSuccessSubtext: "System response verified!"
    };
    ...
    return {
      ...
      themeColor: "#9d4edd",
      icon: "🤖",
      stageTag: "STAGE 2 — INTERMEDIATE",
      ambientSpots: (
        <>
          <div className="absolute top-10 left-1/3 w-[500px] h-[500px] rounded-full bg-indigo-500/5 blur-[120px] pointer-events-none" />
          <div className="absolute bottom-10 right-10 w-80 h-80 rounded-full bg-[#9d4edd]/5 blur-[120px] pointer-events-none" />
        </>
      ),
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-20 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'radial-gradient(circle, rgba(157,78,221,0.06) 1px, transparent 1px)', backgroundSize: '30px 30px' }} />
          <div className="absolute top-1/3 right-1/3 w-2 h-2 rounded-full bg-[#9d4edd] animate-ping" />
        </div>
      ),
      missionSuccessText: "🤖 SYSTEM ONLINE!",
      missionSuccessSubtext: "Autonomous routines operational!"
    };
  } else if (level.stage === 3) {
    return {
      name: "AI & Network Core",
      themeColor: "#c77dff",
      icon: "🌐",
      stageTag: "STAGE 3 — ADVANCED",
      ambientSpots: (
        <>
          <div className="absolute top-20 right-10 w-96 h-96 rounded-full bg-fuchsia-500/5 blur-[120px] pointer-events-none" />
          <div className="absolute bottom-10 left-10 w-[450px] h-[450px] rounded-full bg-[#c77dff]/5 blur-[120px] pointer-events-none" />
        </>
      ),
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-20 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'linear-gradient(rgba(199,125,255,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(199,125,255,0.06) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        </div>
      ),
      missionSuccessText: "🌐 NETWORK SECURED!",
      missionSuccessSubtext: "Digital defense matrix active!"
    };
  } else {
    return {
      name: "Future Technology Matrix",
      themeColor: "#7b2fff",
      icon: "🔮",
      ...
      ambientSpots: (
        <>
          ...
          <div className="absolute bottom-10 right-10 w-[500px] h-[500px] rounded-full bg-[#7b2fff]/5 blur-[120px] pointer-events-none" />
        </>
      ),
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-25 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'radial-gradient(circle, rgba(123,47,255,0.06) 2px, transparent 2px)', backgroundSize: '40px 40px' }} />
        </div>
      ),
      missionSuccessText: "🔮 TECHNOLOGY MASTERED!",
      missionSuccessSubtext: "You have conquered digital intelligence!"
    };
  }
};

// ── STUDENT-FRIENDLY LEVEL TEXTS ────────────────────────
const getSimplifiedLevelTexts = (level: TechLevel) => {
  ...
  return {
    ...
    questionLabel: level.gameMechanic === 'CoordinateGame' ? 'Navigate the drone to safety!' : 'What is your answer?',
...
export const TechnologyWorld: React.FC = () => {
  ...
  const [activeLevel, setActiveLevel] = useState<TechLevel | null>(null);
  ...
  const techLevelsCount = TECHNOLOGY_LEVELS.length;
  const completedTechLevels = TECHNOLOGY_LEVELS.filter(l => state.completedStageIds.includes(l.id));
  const overallTechProgress = Math.round((completedTechLevels.length / techLevelsCount) * 100);

  const isStageCompleted = (stageNum: number) => state.completedStageIds.includes(`tech-${stageNum}-10`);
  const isStageUnlocked = (stageNum: number) => stageNum === 1 || isStageCompleted(stageNum - 1);
  const isLevelUnlocked = (level: TechLevel) => {
    if (!isStageUnlocked(level.stage)) return false;
    if (level.levelNumber === 1) return true;
    return state.completedStageIds.includes(`tech-${level.stage}-${level.levelNumber - 1}`);
  };
  ...
  const handleEnterLevel = (level: TechLevel) => {
  ...
  const handleVerifyAnswer = () => {
    ...
    if (correct) {
      ...
      setIsCorrect(false);
      setFeedbackText(explanation || 'Not quite — check your logic and try again!');
      setAttempts(a => a + 1);
  ...
  const handleVerifyBossPhase = () => {
    ...
    if (phase.gameMechanic === 'CoordinateGame') {
      ...
      if (ok) correct = true;
      else explanation = 'Distribution error. Check ratios and try again!';
    }
    ...
    if (correct) {
      ...
      if (bossPhaseIdx < 3) {
        ...
        setIsCorrect(true);
        setFeedbackText('All 4 phases complete! Technology system fully operational!');
        setShowFeedback(true);
        setShowXpPop(true);
        setTimeout(() => setShowXpPop(false), 2500);
      }
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col justify-between overflow-x-hidden text-slate-100 select-none"
      style={{ background: 'radial-gradient(circle at 50% 50%, #0d021f 0%, #03010a 100%)', fontFamily: 'var(--font-body)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `linear-gradient(rgba(123,47,255,0.015) 1px, transparent 1px), linear-gradient(90deg, rgba(123,47,255,0.015) 1px, transparent 1px)`, backgroundSize: '24px 24px', zIndex: 1, pointerEvents: 'none' }} />
      <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-purple-500/5 blur-[90px] pointer-events-none" style={{ zIndex: 1 }} />
      <div className="absolute bottom-1/3 right-1/4 w-96 h-96 rounded-full bg-[#7b2fff]/5 blur-[90px] pointer-events-none" style={{ zIndex: 1 }} />

      {/* Header */}
      <header className="relative z-10 w-full px-6 py-5 flex items-center justify-between border-b border-purple-500/10 bg-[#06020c]/70 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg border border-purple-500/30 flex items-center justify-center bg-purple-500/5">
            <Cpu size={18} className="text-[#7b2fff]" />
          </div>
          <div className="flex flex-col">
            <span className="font-display text-sm tracking-[0.2em] font-bold text-slate-100 leading-none">TECHNOLOGY</span>
            <span className="text-[10px] tracking-widest text-[#7b2fff] uppercase mt-0.5">TESSERACT STEM HERO</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex flex-col text-right">
            ...
            <span className="font-display text-sm text-[#7b2fff] mt-0.5 font-bold">{completedTechLevels.length} / 40 ({overallTechProgress}%)</span>
          </div>
          <button onClick={handleToggleMute} className="w-9 h-9 rounded-lg border border-slate-800 flex items-center justify-center bg-[#0d0a1a] text-slate-400 hover:text-slate-200 transition-all cursor-pointer">
          ...
          }} className="px-4 py-2 rounded-lg border border-slate-800 bg-[#0d0a1a] hover:border-slate-700 text-xs font-display tracking-widest uppercase text-slate-300 transition-all cursor-pointer flex items-center gap-1.5">
        ...
        <AnimatePresence mode="wait">
          ...
          {viewMode === 'domain-map' && (
            <motion.div key="domain-map" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }}
              ...
              {/* Stone card */}
              <div className="w-full lg:w-5/12 flex flex-col justify-between p-6 rounded-2xl border border-purple-500/10 bg-[#0c0517]/80 backdrop-blur-md shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-24 h-24 bg-purple-500/5 rounded-full blur-2xl pointer-events-none" />
                <div>
                  <span className="font-display text-xs tracking-widest text-[#7b2fff] uppercase block mb-1">Central Relic</span>
                  <h2 className="font-display text-xl tracking-wider text-slate-100 uppercase">The Circuit Stone</h2>
                  <p className="text-sm text-slate-400 mt-2 leading-relaxed">Master programming, logic gates, robotics, data, cybersecurity, and AI. Complete all 4 Stage Bosses to unlock the Technology Stone!</p>
                </div>
                <div className="my-8 flex justify-center items-center relative py-6">
                  <motion.div animate={{ rotate: 360 }} transition={{ duration: 25, repeat: Infinity, ease: 'linear' }} className="absolute w-44 h-44 rounded-full border border-purple-500/15 border-dashed" />
                  <motion.div animate={{ rotate: -360 }} transition={{ duration: 15, repeat: Infinity, ease: 'linear' }} className="absolute w-36 h-36 rounded-full border border-[#7b2fff]/10 border-dotted" />
                  <div className="relative w-28 h-28 flex items-center justify-center">
                    <svg className="absolute w-full h-full" viewBox="0 0 100 100">
                      {[0, 1, 2, 3].map(i => {
                        ...
                        return <path key={i} d={paths[i]} fill={completed ? 'url(#techGlow)' : '#100624'} stroke="#7b2fff" strokeWidth="1" className={completed ? 'animate-pulse' : 'opacity-40'} />;
                      })}
                      <defs>
                        <radialGradient id="techGlow" cx="50%" cy="50%" r="50%">
                          <stop offset="0%" stopColor="#7b2fff" stopOpacity="0.8" />
                          <stop offset="100%" stopColor="#3c108f" stopOpacity="0.4" />
                        </radialGradient>
                      </defs>
                    </svg>
                    <div className="z-10 w-10 h-10 rounded-full bg-black/80 border border-purple-500/40 flex items-center justify-center shadow-lg">
                      <Cpu size={16} className="text-[#7b2fff]" />
                    </div>
                  </div>
                </div>
                <div className="border-t border-slate-800/40 pt-4 flex justify-between items-center text-sm text-slate-500">
                  ...
                  <span className="font-display text-[#7b2fff] font-bold">{[1,2,3,4].filter(n => isStageCompleted(n)).length} / 4 SECURED</span>
                </div>
              </div>
              ...
              <div className="w-full lg:w-7/12 flex flex-col gap-4">
                {[
                  { num: 1, title: 'STAGE 1 — FOUNDATIONS', desc: 'Learn computer basics, binary numbers, logic gates, inputs/outputs, sensors, simple code, loops, and debugging.', locked: false },
                  { num: 2, title: 'STAGE 2 — INTERMEDIATE', desc: 'Master variables, complex logic, robot factory loops, functions, data types, smart sensors, networks, and automation.', locked: !isStageUnlocked(2) },
                  { num: 3, title: 'STAGE 3 — ADVANCED', desc: 'Explore algorithms, data processing, introduction to AI/ML, computer vision, voice assistants, IoT, and cybersecurity.', locked: !isStageUnlocked(3) },
                  { num: 4, title: 'STAGE 4 — MASTERY', desc: 'Master integrated technology systems, search algorithms, AI classification, smart factory automation, and digital defense.', locked: !isStageUnlocked(4) }
                ].map(st => {
                  const completed = isStageCompleted(st.num);
                  return (
                    <div key={st.num} className={`p-5 rounded-2xl border transition-all duration-300 relative overflow-hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${completed ? 'border-purple-500/20 bg-purple-500/5' : st.locked ? 'border-slate-800 bg-[#05020c]/40 opacity-55' : 'border-purple-500/20 bg-[#0c0517]/80 hover:border-purple-500/40 hover:bg-[#120824]/80'}`}>
                      <div className="max-w-[420px]">
                        <div className="flex items-center gap-2">
                          <span className="font-display text-xs tracking-wider font-semibold text-[#7b2fff] uppercase">STAGE {st.num}</span>
                          {completed && <span className="px-2 py-0.5 rounded bg-purple-500/10 text-purple-400 font-display text-xs font-bold tracking-widest uppercase">✓ Completed</span>}
                        </div>
                      ...
                      <div className="flex sm:flex-col items-end gap-2 justify-between shrink-0">
                        {st.locked ? (
                          ...
                          <button onClick={() => handleEnterStage(st.num)} className="px-5 py-2.5 rounded-xl border border-purple-500 bg-[#7b2fff]/10 text-[#7b2fff] font-display text-xs font-bold tracking-widest uppercase hover:bg-[#7b2fff]/20 transition-all cursor-pointer shadow-[0_0_10px_rgba(123,47,255,0.1)]">
                            Enter Stage
                          </button>
                        )}
                      </div>
          ...
          {viewMode === 'stage-levels' && (
            <motion.div key="stage-levels" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="text-center mb-8 max-w-xl">
                <span className="font-display text-xs tracking-widest text-[#7b2fff] uppercase block mb-1">STAGE {selectedStageNum}</span>
                <h1 className="font-display text-3xl tracking-wider text-white uppercase">
                  {selectedStageNum === 1 && 'Foundations'}{selectedStageNum === 2 && 'Intermediate'}{selectedStageNum === 3 && 'Advanced'}{selectedStageNum === 4 && 'Mastery'}
                </h1>
                <p className="text-sm text-slate-400 mt-2">Complete each technology mission in order. Boss unlocks after all 9 missions!</p>
              </div>
                ...
                {getLevelsByStage(selectedStageNum).map(level => {
                  ...
                  return (
                    <button key={level.id} disabled={!unlocked} onClick={() => handleEnterLevel(level)}
                      className={`aspect-square p-4 rounded-xl border flex flex-col justify-between text-left transition-all duration-300 relative overflow-hidden ${completed ? 'border-purple-500/25 bg-purple-500/5 hover:bg-purple-500/10' : isActive ? 'border-[#7b2fff] bg-[#7b2fff]/5 hover:bg-[#7b2fff]/10 shadow-[0_0_15px_rgba(123,47,255,0.15)] animate-pulse' : 'border-slate-800 bg-[#05020c]/60 opacity-40 cursor-default'}`}>
                      <div className="flex justify-between items-center w-full">
                        <span className="font-display text-xs font-bold text-slate-500 tracking-wider">LV {level.levelNumber}</span>
                        {completed ? <CheckCircle size={14} className="text-purple-400" /> : level.isBoss ? <Award size={14} className="text-[#7b2fff] animate-bounce" /> : !unlocked ? <Lock size={12} className="text-slate-600" /> : null}
                      </div>
                      <div>
              ...
              <div className="w-full p-4 rounded-xl border border-slate-800 bg-[#080312]/80 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          {viewMode === 'gameplay' && activeLevel && (
            <motion.div key="gameplay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50">
              {(() => {
                ...
                const stageLevels = TECHNOLOGY_LEVELS.filter(l => l.stage === activeLevel.stage);
                ...
                if (!missionStarted) {
                  return (
                    <div className="fixed inset-0 z-50 flex flex-col overflow-hidden"
                      style={{ background: `radial-gradient(ellipse at 60% 40%, ${env.themeColor}08 0%, #020108 70%)` }}>
                      ...
                      <div className="relative z-20 w-full px-6 py-4 flex items-center justify-between border-b border-white/5 bg-black/30 backdrop-blur-sm shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-[#7b2fff] tracking-widest uppercase font-display font-bold">MISSION {String(activeLevel.levelNumber).padStart(2, '0')}</span>
                        </div>
                      ...
                      <div className="relative z-10 flex-1 flex flex-col lg:flex-row items-stretch overflow-hidden">
                        ...
                        <div className="lg:w-5/12 flex flex-col justify-between p-8 lg:p-10 border-r border-white/5 bg-black/20">
                          <div className="flex items-center gap-3 mb-6">
                            <div className="w-3 h-3 rounded-full bg-[#7b2fff] animate-pulse shadow-[0_0_8px_#7b2fff]" />
                            <span className="text-xs text-[#7b2fff] font-display tracking-widest uppercase font-bold">CYBER COMMS — ACTIVE</span>
                          </div>
                          <div className="flex-1 flex flex-col justify-center gap-6">
                            <div className="relative rounded-2xl overflow-hidden border-2 shadow-2xl" style={{ borderColor: `${env.themeColor}40`, boxShadow: `0 0 40px ${env.themeColor}15` }}>
                              ...
                              <div className="absolute top-3 left-3 z-20 flex items-center gap-1.5 bg-black/70 px-3 py-1 rounded-lg">
                                <span className="w-2 h-2 rounded-full bg-[#7b2fff] animate-ping" />
                                <span className="text-xs text-[#7b2fff] font-mono font-bold font-display">MORALES · ONLINE</span>
                              </div>
                ...
                return (
                  <div className="fixed inset-0 z-50 flex flex-col w-full h-screen overflow-hidden text-slate-100"
                    style={{ background: `radial-gradient(ellipse at 50% 30%, ${env.themeColor}06 0%, #020108 60%)` }}>
                    {env.environmentalDecoration}
                    {env.ambientSpots}
                    ...
                    <header className="relative z-20 w-full h-16 px-5 flex items-center justify-between border-b border-white/5 bg-black/50 backdrop-blur-md shrink-0">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="font-display text-sm font-bold text-white leading-none tracking-wider">TECHNOLOGY · STAGE {activeLevel.stage}</span>
                          <span className="text-xs tracking-widest text-[#7b2fff] leading-none mt-0.5 font-bold uppercase">MISSION {String(activeLevel.levelNumber).padStart(2, '0')} — {texts.briefingTitle}</span>
                        </div>
                      </div>
                        ...
                        {stageLevels.map(l => {
                          ...
                          return (
                            <div key={l.id} title={`Mission ${l.levelNumber}`}
                              className={`rounded-full flex items-center justify-center font-display font-bold transition-all ${isCurrent ? 'w-7 h-7 text-xs text-white shadow-lg' : isDone ? 'w-5 h-5 text-[9px]' : 'w-4 h-4 text-[8px]'}`}
                              style={{ background: isCurrent ? env.themeColor : isDone ? `${env.themeColor}30` : '#1a0a2e', border: isCurrent ? `2px solid ${env.themeColor}` : isDone ? `1px solid ${env.themeColor}50` : '1px solid #2a1a3e', boxShadow: isCurrent ? `0 0 10px ${env.themeColor}60` : 'none', color: isCurrent ? '#fff' : isDone ? env.themeColor : '#4a3a6a' }}>
                    ...
                    <div className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden" style={{ height: 'calc(100vh - 64px)' }}>
                      ...
                      <div className="lg:col-span-3 border-r border-white/5 bg-black/30 backdrop-blur-sm flex flex-col p-5 gap-5 overflow-y-auto">
                        <div className="relative rounded-2xl overflow-hidden border-2 shadow-xl" style={{ borderColor: `${env.themeColor}35`, boxShadow: `0 0 25px ${env.themeColor}12` }}>
                          <div className="absolute top-3 left-3 z-20 flex items-center gap-1.5 bg-black/80 px-2.5 py-1 rounded-lg">
                            <span className="w-2 h-2 rounded-full bg-[#7b2fff] animate-ping" />
                            <span className="text-xs text-[#7b2fff] font-mono font-bold">ACTIVE</span>
                          </div>
                        ...
                        <div className="flex items-center gap-0.5 justify-center h-6 px-2">
                          {[...Array(14)].map((_, i) => (
                            <div key={i} className="flex-1 rounded-full transition-all"
                              style={{ height: showFeedback ? `${6 + Math.abs(Math.sin(i * 0.7)) * 14}px` : '3px', background: isCorrect && showFeedback ? '#7b2fff' : showFeedback ? '#ef4444' : env.themeColor, transitionDelay: `${i * 30}ms`, opacity: showFeedback ? 0.8 : 0.3 }} />
                          ))}
                        </div>
                      ...
                      <div className="lg:col-span-6 flex flex-col gap-4 p-5 overflow-y-auto">
                        ...
                        <div className="flex items-center justify-between p-3 rounded-xl border bg-black/40" style={{ borderColor: `${env.themeColor}20` }}>
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="text-sm font-display font-bold uppercase tracking-wider text-slate-200">
                              {showFeedback && isCorrect ? env.missionSuccessText : `💻 ${env.name} — SYSTEM ACTIVE`}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-display text-slate-500 uppercase font-bold">Signal</span>
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center rounded-2xl border-2 p-6 gap-6 relative overflow-hidden"
                          style={{ borderColor: `${env.themeColor}25`, background: `radial-gradient(ellipse at 50% 0%, ${env.themeColor}06 0%, #020108 70%)` }}>
                          ...
                          {activeLevel.gameMechanic === 'PatternGame' && activeLevel.gameData.sequence && (
                            <div className="flex flex-col items-center gap-6 w-full">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Code Sequence</p><p className="text-lg text-slate-300">{texts.story}</p></div>
                          ...
                          {activeLevel.gameMechanic === 'EquationGame' && (
                            <div className="flex flex-col items-center gap-6 w-full">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Code Calculation</p><p className="text-base text-slate-300">{texts.story}</p></div>
                          ...
                          {activeLevel.gameMechanic === 'LogicGame' && (
                            <div className="flex flex-col gap-5 w-full max-w-lg">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Tech Challenge</p></div>
                          ...
                          {activeLevel.gameMechanic === 'OptimizationGame' && activeLevel.gameData.optionsToChoose && (
                            <div className="flex flex-col gap-5 w-full max-w-lg">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Best System Choice</p><p className="text-base text-slate-300">{texts.story}</p></div>
                              <div className="flex flex-col gap-3">
                          ...
                          {(activeLevel.gameMechanic === 'ResourceAllocationGame' || activeLevel.gameMechanic === 'DragDropGame') && (
                            <div className="flex flex-col gap-5 w-full max-w-md">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">System Allocation</p><p className="text-base text-slate-300">{texts.story}</p></div>
                              <div className="flex flex-col gap-3">
                          ...
                          {activeLevel.gameMechanic === 'CoordinateGame' && (
                            <div className="flex flex-col items-center gap-5 w-full">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Drone Path Grid</p><p className="text-base text-slate-300">{texts.story}</p></div>
                                    ...
                                    return Array.from({ length: gs }).map((_, x) => {
                                      ...
                                      return (
                                        <div key={`${x}-${y}`} className="w-16 h-16 rounded-xl flex flex-col items-center justify-center font-display font-bold border transition-all"
                                          ...
                                          <span className="text-xl">{isDrone ? '🤖' : isTgt ? '📍' : isObs ? '⚠️' : ''}</span>
                              ...
                              return (
                                <div key={l.id} className="flex items-center gap-2">
                                  <div className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold font-display shrink-0"
                                    style={{ background: isCurrent ? env.themeColor : isDone ? `${env.themeColor}30` : '#1a0a2e', color: isCurrent ? '#fff' : isDone ? env.themeColor : '#4a3a6a', border: `1px solid ${isCurrent ? env.themeColor : isDone ? `${env.themeColor}50` : '#2a1a3e'}` }}>
                    ...
                    <AnimatePresence>
                      {showFeedback && isCorrect && (
                        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                          ...
                          style={{ background: `radial-gradient(ellipse at 50% 40%, ${env.themeColor}12 0%, rgba(2,1,8,0.97) 70%)`, backdropFilter: 'blur(16px)' }}>
        ...
        {showCeremony && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
              className="absolute w-80 h-80 rounded-full border border-purple-500/10 border-dashed pointer-events-none" />
            <motion.div animate={{ rotate: -360 }} transition={{ duration: 18, repeat: Infinity, ease: 'linear' }}
              className="absolute w-64 h-64 rounded-full border border-[#7b2fff]/8 border-dotted pointer-events-none" />
            <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.85, opacity: 0 }} transition={{ delay: 0.2, type: 'spring', bounce: 0.3 }}
              ...
              <motion.div animate={{ scale: [1, 1.1, 1], rotate: [0, 180, 360] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
                className="w-24 h-24 bg-gradient-to-tr from-purple-600 to-[#7b2fff] rounded-2xl flex items-center justify-center shadow-[0_0_60px_rgba(123,47,255,0.6)]">
                <Cpu size={42} className="text-white" strokeWidth={2.5} />
              </motion.div>
              <div>
                <span className="font-display text-xs tracking-[0.3em] text-[#7b2fff] uppercase block mb-2">⚡ Tesseract Sync Activated ⚡</span>
                <h2 className="font-display text-2xl tracking-widest text-white uppercase font-extrabold">{earnedFragment} Secured!</h2>
              </div>
              <div className="w-16 h-0.5 bg-[#7b2fff] rounded-full" />
              <div className="flex items-center gap-3">
                {[1,2,3,4].map(n => (
                  <div key={n} className="flex flex-col items-center gap-1.5">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center border-2"
                      style={{ borderColor: isStageCompleted(n) ? '#7b2fff' : '#1e1e2e', background: isStageCompleted(n) ? 'rgba(123,47,255,0.15)' : '#06020e', boxShadow: isStageCompleted(n) ? '0 0 15px rgba(123,47,255,0.4)' : 'none' }}>
                      {isStageCompleted(n) ? <Shield size={18} className="text-[#7b2fff]" /> : <Lock size={16} className="text-slate-600" />}
                    </div>
                    <span className="text-xs text-slate-500 font-display">STG {n}</span>
                  </div>
                ))}
              </div>
              <p className="text-sm text-slate-400 leading-relaxed">You've unlocked a Technology Stone Fragment! Complete all 4 stages to form the Circuit Stone.</p>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} onClick={handleCloseCeremony}
                className="px-10 py-4 rounded-2xl border-2 border-[#7b2fff] bg-[#7b2fff]/15 hover:bg-[#7b2fff]/25 text-[#7b2fff] font-display text-base font-extrabold tracking-widest uppercase transition-all shadow-[0_0_25px_rgba(123,47,255,0.3)] cursor-pointer">
                ⚡ Continue Adventure
              </motion.button>
            </motion.div>
          </motion.div>
      ...
      <footer className="relative z-10 w-full px-6 py-3 flex justify-between items-center text-xs text-slate-600 border-t border-white/5 bg-black/20">
        <span>S.H.I.E.L.D. · TECHNOLOGY DOMAIN</span>
...
export default TechnologyWorld;
