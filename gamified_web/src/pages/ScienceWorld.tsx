...
import type { SciLevel } from '../data/scienceLevels';
import { audioSynth } from '../utils/audio';

// ── ENVIRONMENT PER STAGE ─────────────────────────────────
const getMissionEnvironment = (level: SciLevel) => {
  if (level.stage === 1) {
    return {
      name: "Research Laboratory",
      themeColor: "#00e5ff",
      icon: "🔬",
      stageTag: "STAGE 1 — DISCOVER",
      ambientSpots: (
        <>
          <div className="absolute top-10 left-10 w-96 h-96 rounded-full bg-[#00e5ff]/5 blur-[120px] pointer-events-none" />
      ...
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-20 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'linear-gradient(rgba(0,229,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(0,229,255,0.04) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
          <div className="absolute top-1/4 left-1/3 w-2 h-2 rounded-full bg-[#00e5ff] animate-ping" />
          <div className="absolute top-2/3 right-1/4 w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping" style={{ animationDelay: '1.5s' }} />
        </div>
      ),
      missionSuccessText: "🔬 EXPERIMENT COMPLETE!",
      missionSuccessSubtext: "The laboratory data is secured!"
    };
    ...
    return {
      ...
      themeColor: "#00bcd4",
      icon: "⚡",
      stageTag: "STAGE 2 — UNDERSTAND",
      ambientSpots: (
        <>
          <div className="absolute top-10 left-1/3 w-[500px] h-[500px] rounded-full bg-teal-500/5 blur-[120px] pointer-events-none" />
          <div className="absolute bottom-10 right-10 w-80 h-80 rounded-full bg-[#00bcd4]/5 blur-[120px] pointer-events-none" />
        </>
      ),
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-20 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'radial-gradient(circle, rgba(0,188,212,0.06) 1px, transparent 1px)', backgroundSize: '30px 30px' }} />
          <div className="absolute top-1/3 right-1/3 w-2 h-2 rounded-full bg-[#00bcd4] animate-ping" />
        </div>
      ),
      missionSuccessText: "⚡ POWER STATION SAVED!",
      missionSuccessSubtext: "Energy systems are stable!"
    };
  } else if (level.stage === 3) {
    return {
      name: "Ecosystem Research Center",
      themeColor: "#4caf50",
      icon: "🌿",
      stageTag: "STAGE 3 — LIFE & EARTH",
      ambientSpots: (
        <>
          <div className="absolute top-20 right-10 w-96 h-96 rounded-full bg-green-500/5 blur-[120px] pointer-events-none" />
          <div className="absolute bottom-10 left-10 w-[450px] h-[450px] rounded-full bg-[#4caf50]/5 blur-[120px] pointer-events-none" />
        </>
      ),
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-20 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'linear-gradient(rgba(76,175,80,0.06) 1px, transparent 1px), linear-gradient(90deg, rgba(76,175,80,0.06) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        </div>
      ),
      missionSuccessText: "🌿 ECOSYSTEM RESTORED!",
      missionSuccessSubtext: "Nature is back in balance!"
    };
  } else {
    return {
      name: "Scientific Mastery Lab",
      themeColor: "#00e5ff",
      icon: "🧬",
      ...
      ambientSpots: (
        <>
          ...
          <div className="absolute bottom-10 right-10 w-[500px] h-[500px] rounded-full bg-[#00e5ff]/5 blur-[120px] pointer-events-none" />
        </>
      ),
      environmentalDecoration: (
        <div className="absolute inset-0 pointer-events-none opacity-25 z-0">
          <div className="w-full h-full" style={{ backgroundImage: 'radial-gradient(circle, rgba(0,229,255,0.06) 2px, transparent 2px)', backgroundSize: '40px 40px' }} />
        </div>
      ),
      missionSuccessText: "🧬 SCIENCE MASTERED!",
      missionSuccessSubtext: "Your scientific knowledge is complete!"
    };
  }
};

// ── STUDENT-FRIENDLY LEVEL TEXTS ────────────────────────
const getSimplifiedLevelTexts = (level: SciLevel) => {
  ...
  return {
    ...
    questionLabel: level.gameMechanic === 'CoordinateGame' ? 'Guide the probe to safety!' : 'What is your answer?',
...
export const ScienceWorld: React.FC = () => {
  ...
  const sciLevelsCount = SCIENCE_LEVELS.length;
  const completedSciLevels = SCIENCE_LEVELS.filter(l => state.completedStageIds.includes(l.id));
  const overallSciProgress = Math.round((completedSciLevels.length / sciLevelsCount) * 100);

  const isStageCompleted = (stageNum: number) => state.completedStageIds.includes(`sci-${stageNum}-10`);
  const isStageUnlocked = (stageNum: number) => stageNum === 1 || isStageCompleted(stageNum - 1);
  const isLevelUnlocked = (level: SciLevel) => {
    if (!isStageUnlocked(level.stage)) return false;
    if (level.levelNumber === 1) return true;
    return state.completedStageIds.includes(`sci-${level.stage}-${level.levelNumber - 1}`);
  };
  ...
  const handleEnterLevel = (level: SciLevel) => {
  ...
  const handleVerifyAnswer = () => {
    ...
    if (correct) {
      ...
      setIsCorrect(false);
      setFeedbackText(explanation || 'Not quite — look at the clues and try again!');
      setAttempts(a => a + 1);
  ...
  const handleVerifyBossPhase = () => {
    ...
    if (phase.gameMechanic === 'CoordinateGame') {
      ...
      if (currentCoordinate[0] === tp[0] && currentCoordinate[1] === tp[1]) correct = true;
      else explanation = 'The probe missed the target. Try again!';
      ...
      else explanation = 'Wrong choice. Review the clues and try again.';
    } else if (phase.gameMechanic === 'OptimizationGame') {
      if (selectedOption === phase.gameData.optimalChoiceName) correct = true;
      else explanation = 'Not the best choice. Compare the options again.';
      ...
      if (ok) correct = true;
      else explanation = 'Distribution is off. Try again!';
    }
    ...
    if (correct) {
      ...
      if (bossPhaseIdx < 3) {
        ...
        setIsCorrect(true);
        setFeedbackText('All 4 phases complete! You saved the day!');
        setShowFeedback(true);
        setShowXpPop(true);
        setTimeout(() => setShowXpPop(false), 2500);
      }
  ...
  return (
    <div className="relative min-h-screen w-full flex flex-col justify-between overflow-x-hidden text-slate-100 select-none"
      style={{ background: 'radial-gradient(circle at 50% 50%, #020c0d 0%, #010105 100%)', fontFamily: 'var(--font-body)' }}>
      <div style={{ position: 'absolute', inset: 0, backgroundImage: `linear-gradient(rgba(0,229,255,0.015) 1px, transparent 1px), linear-gradient(90deg, rgba(0,229,255,0.015) 1px, transparent 1px)`, backgroundSize: '24px 24px', zIndex: 1, pointerEvents: 'none' }} />
      <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-cyan-500/5 blur-[90px] pointer-events-none" style={{ zIndex: 1 }} />
      <div className="absolute bottom-1/3 right-1/4 w-96 h-96 rounded-full bg-teal-500/5 blur-[90px] pointer-events-none" style={{ zIndex: 1 }} />

      {/* Header */}
      <header className="relative z-10 w-full px-6 py-5 flex items-center justify-between border-b border-cyan-500/10 bg-[#020208]/70 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg border border-cyan-500/30 flex items-center justify-center bg-cyan-500/5">
            <FlaskConical size={18} className="text-[#00e5ff]" />
          </div>
          <div className="flex flex-col">
            <span className="font-display text-sm tracking-[0.2em] font-bold text-slate-100 leading-none">SCIENCE</span>
            <span className="text-[10px] tracking-widest text-[#00e5ff] uppercase mt-0.5">TESSERACT STEM HERO</span>
          </div>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="hidden md:flex flex-col text-right">
            ...
            <span className="font-display text-sm text-[#00e5ff] mt-0.5 font-bold">{completedSciLevels.length} / 40 ({overallSciProgress}%)</span>
          </div>
          <button onClick={handleToggleMute} className="w-9 h-9 rounded-lg border border-slate-800 flex items-center justify-center bg-[#0d0d1a] text-slate-400 hover:text-slate-200 transition-all cursor-pointer">
          ...
          }} className="px-4 py-2 rounded-lg border border-slate-800 bg-[#0d0d1a] hover:border-slate-700 text-xs font-display tracking-widest uppercase text-slate-300 transition-all cursor-pointer flex items-center gap-1.5">
            ...
            <motion.div key="domain-map" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }}
              ...
              <div className="w-full lg:w-5/12 flex flex-col justify-between p-6 rounded-2xl border border-cyan-500/10 bg-[#080812]/80 backdrop-blur-md shadow-2xl relative overflow-hidden">
                ...
                <div className="my-8 flex justify-center items-center relative py-6">
                  ...
                  <motion.div animate={{ rotate: -360 }} transition={{ duration: 15, repeat: Infinity, ease: 'linear' }} className="absolute w-36 h-36 rounded-full border border-[#00e5ff]/10 border-dotted" />
                  <div className="relative w-28 h-28 flex items-center justify-center">
                    <svg className="absolute w-full h-full" viewBox="0 0 100 100">
                      {[0, 1, 2, 3].map(i => {
                        ...
                        return <path key={i} d={paths[i]} fill={completed ? 'url(#sciGlow)' : '#0f0f22'} stroke="#00e5ff" strokeWidth="1" className={completed ? 'animate-pulse' : 'opacity-40'} />;
                      })}
                      <defs>
                        <radialGradient id="sciGlow" cx="50%" cy="50%" r="50%">
                          <stop offset="0%" stopColor="#00e5ff" stopOpacity="0.8" />
                          <stop offset="100%" stopColor="#006677" stopOpacity="0.4" />
                        </radialGradient>
                      </defs>
                    </svg>
                    <div className="z-10 w-10 h-10 rounded-full bg-black/80 border border-cyan-500/40 flex items-center justify-center shadow-lg">
                ...
                {getLevelsByStage(selectedStageNum).map(level => {
                  ...
                  return (
                    <button key={level.id} disabled={!unlocked} onClick={() => handleEnterLevel(level)}
                      className={`aspect-square p-4 rounded-xl border flex flex-col justify-between text-left transition-all duration-300 relative overflow-hidden ${completed ? 'border-cyan-500/25 bg-cyan-500/5 hover:bg-cyan-500/10' : isActive ? 'border-[#00e5ff] bg-[#00e5ff]/5 hover:bg-[#00e5ff]/10 shadow-[0_0_15px_rgba(0,229,255,0.15)] animate-pulse' : 'border-slate-800 bg-[#040409]/60 opacity-40 cursor-default'}`}>
                      <div className="flex justify-between items-center w-full">
                        <span className="font-display text-xs font-bold text-slate-500 tracking-wider">LV {level.levelNumber}</span>
                        {completed ? <CheckCircle size={14} className="text-cyan-400" /> : level.isBoss ? <Award size={14} className="text-[#00e5ff] animate-bounce" /> : !unlocked ? <Lock size={12} className="text-slate-600" /> : null}
                      </div>
                      <div>
                    ...
                    <div className="fixed inset-0 z-50 flex flex-col overflow-hidden"
                      ...
                      <div className="relative z-20 w-full px-6 py-4 flex items-center justify-between border-b border-white/5 bg-black/30 backdrop-blur-sm shrink-0">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-xs text-[#00e5ff] tracking-widest uppercase font-display font-bold">MISSION {String(activeLevel.levelNumber).padStart(2, '0')}</span>
                        </div>
                      ...
                      <div className="relative z-10 flex-1 flex flex-col lg:flex-row items-stretch overflow-hidden">
                        ...
                        <div className="lg:w-5/12 flex flex-col justify-between p-8 lg:p-10 border-r border-white/5 bg-black/20">
                          <div className="flex items-center gap-3 mb-6">
                            <div className="w-3 h-3 rounded-full bg-[#00e5ff] animate-pulse shadow-[0_0_8px_#00e5ff]" />
                  ...
                  <div className="fixed inset-0 z-50 flex flex-col w-full h-screen overflow-hidden text-slate-100"
                    ...
                    <header className="relative z-20 w-full h-16 px-5 flex items-center justify-between border-b border-white/5 bg-black/50 backdrop-blur-md shrink-0">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="font-display text-sm font-bold text-white leading-none tracking-wider">SCIENCE · STAGE {activeLevel.stage}</span>
                          <span className="text-xs tracking-widest text-[#00e5ff] leading-none mt-0.5 font-bold uppercase">MISSION {String(activeLevel.levelNumber).padStart(2, '0')} — {texts.briefingTitle}</span>
                        </div>
                      </div>
                        ...
                        {stageLevels.map(l => {
                          ...
                          return (
                            <div key={l.id} title={`Mission ${l.levelNumber}`}
                              ...
                              style={{ background: isCurrent ? env.themeColor : isDone ? `${env.themeColor}30` : '#1a1a2e', border: isCurrent ? `2px solid ${env.themeColor}` : isDone ? `1px solid ${env.themeColor}50` : '1px solid #2a2a3e', boxShadow: isCurrent ? `0 0 10px ${env.themeColor}60` : 'none', color: isCurrent ? '#000' : isDone ? env.themeColor : '#4a4a6a' }}>
                    ...
                    <div className="relative z-10 flex-1 grid grid-cols-1 lg:grid-cols-12 overflow-hidden" style={{ height: 'calc(100vh - 64px)' }}>
                        ...
                        <div className="flex items-center gap-0.5 justify-center h-6 px-2">
                          {[...Array(14)].map((_, i) => (
                            <div key={i} className="flex-1 rounded-full transition-all"
                              style={{ height: showFeedback ? `${6 + Math.abs(Math.sin(i * 0.7)) * 14}px` : '3px', background: isCorrect && showFeedback ? '#00e5ff' : showFeedback ? '#ef4444' : env.themeColor, transitionDelay: `${i * 30}ms`, opacity: showFeedback ? 0.8 : 0.3 }} />
                          ))}
                        </div>
                      ...
                      <div className="lg:col-span-6 flex flex-col gap-4 p-5 overflow-y-auto">
                        ...
                        <div className="flex items-center justify-between p-3 rounded-xl border bg-black/40" style={{ borderColor: `${env.themeColor}20` }}>
                          <div className="flex items-center gap-2.5">
                            ...
                            <span className="text-sm font-display font-bold uppercase tracking-wider text-slate-200">
                              {showFeedback && isCorrect ? env.missionSuccessText : `🔬 ${env.name} — EXPERIMENT IN PROGRESS`}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-xs font-display text-slate-500 uppercase font-bold">Energy</span>
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center rounded-2xl border-2 p-6 gap-6 relative overflow-hidden"
                          style={{ borderColor: `${env.themeColor}25`, background: `radial-gradient(ellipse at 50% 0%, ${env.themeColor}06 0%, #020209 70%)` }}>
                          ...
                          {activeLevel.gameMechanic === 'PatternGame' && activeLevel.gameData.sequence && (
                            <div className="flex flex-col items-center gap-6 w-full">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Data Pattern</p><p className="text-lg text-slate-300">{texts.story}</p></div>
                          ...
                          {activeLevel.gameMechanic === 'EquationGame' && (
                            <div className="flex flex-col items-center gap-6 w-full">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Science Calculation</p><p className="text-base text-slate-300">{texts.story}</p></div>
                          ...
                          {activeLevel.gameMechanic === 'LogicGame' && (
                            <div className="flex flex-col gap-5 w-full max-w-lg">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Science Challenge</p></div>
                          ...
                          {activeLevel.gameMechanic === 'OptimizationGame' && activeLevel.gameData.optionsToChoose && (
                            <div className="flex flex-col gap-5 w-full max-w-lg">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Best Choice</p><p className="text-base text-slate-300">{texts.story}</p></div>
                              <div className="flex flex-col gap-3">
                          ...
                          {(activeLevel.gameMechanic === 'ResourceAllocationGame' || activeLevel.gameMechanic === 'DragDropGame') && (
                            <div className="flex flex-col gap-5 w-full max-w-md">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Resource Distribution</p><p className="text-base text-slate-300">{texts.story}</p></div>
                              <div className="flex flex-col gap-3">
                          ...
                          {activeLevel.gameMechanic === 'CoordinateGame' && (
                            <div className="flex flex-col items-center gap-5 w-full">
                              <div className="text-center"><p className="text-xs text-slate-500 font-display tracking-widest uppercase font-bold mb-2">Grid Navigation</p><p className="text-base text-slate-300">{texts.story}</p></div>
                              <div className="flex flex-col items-center gap-4">
                                ...
                                <div className="flex flex-col items-center gap-2">
                                  <p className="text-xs text-slate-500 uppercase tracking-widest font-display">Move the probe</p>
        ...
        {showCeremony && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div animate={{ rotate: 360 }} transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
              className="absolute w-80 h-80 rounded-full border border-cyan-500/10 border-dashed pointer-events-none" />
            <motion.div animate={{ rotate: -360 }} transition={{ duration: 18, repeat: Infinity, ease: 'linear' }}
              className="absolute w-64 h-64 rounded-full border border-[#00e5ff]/8 border-dotted pointer-events-none" />
            <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.85, opacity: 0 }} transition={{ delay: 0.2, type: 'spring', bounce: 0.3 }}
              ...
              <motion.div animate={{ scale: [1, 1.1, 1], rotate: [0, 180, 360] }} transition={{ duration: 6, repeat: Infinity, ease: 'easeInOut' }}
                className="w-24 h-24 bg-gradient-to-tr from-cyan-500 to-[#00e5ff] rounded-2xl flex items-center justify-center shadow-[0_0_60px_rgba(0,229,255,0.6)]">
                <FlaskConical size={42} className="text-black" strokeWidth={2.5} />
              </motion.div>
              <div>
                <span className="font-display text-xs tracking-[0.3em] text-[#00e5ff] uppercase block mb-2">⚡ Tesseract Sync Activated ⚡</span>
                <h2 className="font-display text-2xl tracking-widest text-white uppercase font-extrabold">{earnedFragment} Secured!</h2>
              </div>
              <div className="w-16 h-0.5 bg-[#00e5ff] rounded-full" />
              <div className="flex items-center gap-3">
                {[1,2,3,4].map(n => (
                  <div key={n} className="flex flex-col items-center gap-1.5">
                    <div className="w-10 h-10 rounded-lg flex items-center justify-center border-2"
                      style={{ borderColor: isStageCompleted(n) ? '#00e5ff' : '#1e1e2e', background: isStageCompleted(n) ? 'rgba(0,229,255,0.15)' : '#06060e', boxShadow: isStageCompleted(n) ? '0 0 15px rgba(0,229,255,0.4)' : 'none' }}>
                      {isStageCompleted(n) ? <Shield size={18} className="text-[#00e5ff]" /> : <Lock size={16} className="text-slate-600" />}
                    </div>
                    <span className="text-xs text-slate-500 font-display">STG {n}</span>
                  </div>
                ))}
              </div>
              <p className="text-sm text-slate-400 leading-relaxed">You've unlocked a Science Stone Fragment! Complete all 4 stages to form the Luminal Stone.</p>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} onClick={handleCloseCeremony}
                className="px-10 py-4 rounded-2xl border-2 border-[#00e5ff] bg-[#00e5ff]/15 hover:bg-[#00e5ff]/25 text-[#00e5ff] font-display text-base font-extrabold tracking-widest uppercase transition-all shadow-[0_0_25px_rgba(0,229,255,0.3)] cursor-pointer">
                ⚡ Continue Adventure
              </motion.button>
            </motion.div>
          </motion.div>
      ...
      <footer className="relative z-10 w-full px-6 py-3 flex justify-between items-center text-xs text-slate-600 border-t border-white/5 bg-black/20">
        <span>S.H.I.E.L.D. · SCIENCE DOMAIN</span>
