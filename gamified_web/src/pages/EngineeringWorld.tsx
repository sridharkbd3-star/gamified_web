          ...
          {viewMode === 'domain-map' && (
            <motion.div key="domain-map" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }}
              ...
              <div className="w-full lg:w-5/12 flex flex-col justify-between p-6 rounded-2xl border border-amber-500/10 bg-[#170a03]/80 backdrop-blur-md shadow-2xl relative overflow-hidden">
                ...
                <div>
                  <span className="font-display text-xs tracking-widest text-[#ff9500] uppercase block mb-1">Central Relic</span>
                  <h2 className="font-display text-xl tracking-wider text-slate-100 uppercase">The Forge Stone</h2>
                  <p className="text-sm text-slate-400 mt-2 leading-relaxed">Master structures, load balancing, mechanical systems, robotics, circuits, and the design process. Complete all 4 Stage Bosses to unlock the Engineering Stone!</p>
                </div>
                ...
                <div className="border-t border-slate-800/40 pt-4 flex justify-between items-center text-sm text-slate-500">
                  ...
                  <span className="font-display text-[#ff9500] font-bold">{[1,2,3,4].filter(n => isStageCompleted(n)).length} / 4 SECURED</span>
                </div>
              </div>
              ...
              <div className="w-full lg:w-7/12 flex flex-col gap-4">
                {[
                  { num: 1, title: 'STAGE 1 — FOUNDATIONS', desc: 'Learn basic structures, material properties, stability, load testing, forces, levers, wheels, gears, and design thinking.', locked: false },
                  { num: 2, title: 'STAGE 2 — MECHANICAL', desc: 'Master gear trains, pulley systems, levers, mechanical energy transfer, speed control, robot arms, and optimization.', locked: !isStageUnlocked(2) },
                  { num: 3, title: 'STAGE 3 — ELECTRICAL & ROBOTICS', desc: 'Explore series/parallel circuits, sensors, DC motors, feedback loops, robot joint movement, electrical power, and disaster robotics.', locked: !isStageUnlocked(3) },
                  { num: 4, title: 'STAGE 4 — MASTERY', desc: 'Execute the complete engineering design process: problem definition, solution generation, prototyping, testing, failure analysis, and city-scale design.', locked: !isStageUnlocked(4) }
                ].map(st => {
                    ...
                    <div key={st.num} className={`p-5 rounded-2xl border transition-all duration-300 relative overflow-hidden flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ${completed ? 'border-amber-500/20 bg-amber-500/5' : st.locked ? 'border-slate-800 bg-[#0d0502]/40 opacity-55' : 'border-amber-500/20 bg-[#170a03]/80 hover:border-amber-500/40 hover:bg-[#241005]/80'}`}>
                      <div className="max-w-[420px]">
                        <div className="flex items-center gap-2">
                          <span className="font-display text-xs tracking-wider font-semibold text-[#ff9500] uppercase">STAGE {st.num}</span>
                          {completed && <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-400 font-display text-xs font-bold tracking-widest uppercase">✓ Completed</span>}
                        </div>
          ...
          {viewMode === 'stage-levels' && (
            <motion.div key="stage-levels" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="text-center mb-8 max-w-xl">
                <span className="font-display text-xs tracking-widest text-[#ff9500] uppercase block mb-1">STAGE {selectedStageNum}</span>
                <h1 className="font-display text-3xl tracking-wider text-white uppercase">
                  {selectedStageNum === 1 && 'Foundations'}{selectedStageNum === 2 && 'Mechanical'}{selectedStageNum === 3 && 'Robotics'}{selectedStageNum === 4 && 'Mastery'}
                </h1>
                <p className="text-sm text-slate-400 mt-2">Complete each engineering mission in order. Boss unlocks after all 9 missions!</p>
              </div>
                          ...
                          <div className="flex-1 flex flex-col justify-center gap-6">
                            <div className="relative rounded-2xl overflow-hidden border-2 shadow-2xl" style={{ borderColor: `${env.themeColor}40`, boxShadow: `0 0 40px ${env.themeColor}15` }}>
                              ...
                              <div className="absolute top-3 left-3 z-20 flex items-center gap-1.5 bg-black/70 px-3 py-1 rounded-lg">
                                ...
                                <span className="text-xs text-[#ff9500] font-mono font-bold font-display">MORALES · ONLINE</span>
                              </div>
                      ...
                      <div className="lg:col-span-3 border-r border-white/5 bg-black/30 backdrop-blur-sm flex flex-col p-5 gap-5 overflow-y-auto">
                        <div className="relative rounded-2xl overflow-hidden border-2 shadow-xl" style={{ borderColor: `${env.themeColor}35`, boxShadow: `0 0 25px ${env.themeColor}12` }}>
                          <div className="absolute top-3 left-3 z-20 flex items-center gap-1.5 bg-black/80 px-2.5 py-1 rounded-lg">
                            <span className="w-2 h-2 rounded-full bg-[#ff9500] animate-ping" />
                            <span className="text-xs text-[#ff9500] font-mono font-bold">ACTIVE</span>
                          </div>
                          ...
                          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                            <p className="text-xs font-display text-white font-bold tracking-wider uppercase">{showHint ? '🔮 FUTURE MORALES' : '⚙️ AGENT MORALES'}</p>
                          </div>
                        </div>
                              ...
                              return (
                                <div key={l.id} className="flex items-center gap-2">
                                  <div className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold font-display shrink-0"
                                    style={{ background: isCurrent ? env.themeColor : isDone ? `${env.themeColor}30` : '#2e1a0a', color: isCurrent ? '#000' : isDone ? env.themeColor : '#6a4a3a', border: `1px solid ${isCurrent ? env.themeColor : isDone ? `${env.themeColor}50` : '#3e2a1a'}` }}>
